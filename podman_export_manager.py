...
class PodmanExportManager:
  ...
  def export_volumes(self, volumes: list[str], containers: list[str]) -> None:
    ...
    self.log.info("stopping services %s", " ".join(containers))
    ...
    for v in volumes:
      self.log.info("exporting named volume %s", v)
      error_encountered = (
        not self._podman_export_volume(v, self.export_path) or error_encountered
      )

    for c in containers:
      self.log.info("starting service %s", c)
  ...
  def _systemctl_is_active(self, service: str) -> bool:
    ...
    self.log.debug("status of %s: %s", service, status)
  ...
  def _systemctl_start(self, service: str) -> None:
    ...
    if not self._systemctl_is_active(service):
      args = [*self._systemctl, "restart", service]
      self.log.debug("running command: %s", " ".join(args))
